    ...
    public partial class X : Form
    {
        ...
        public class GraphQLFacebook
        {
            ...
            public GraphQLFacebook(string uid)
            {
                ...
                httpClient.DefaultRequestHeaders.Add("Cookie", "locale=th_TH"); //đặt ngôn ngữ facebook thành ngôn ngữ Việt Nam
        ...
        private async void getlist__Click(object sender, EventArgs e)
        {
            if (getlist_.Text == "GET")
            {
                getlist_.Text = "STOP";
                isStop = false;
                await Task.Run(async () =>
                {
                    string[] uids = idlist_.Lines;
                    for (var i = 0; i < uids.Length; i++)
                    {
                        ...
                        if (!string.IsNullOrEmpty(uid))
                        {
                            ...
                            row.Cells[4].Value = fb.ResultInfo.Email;
                            row.Cells[5].Value = fb.ResultInfo.Phone;
                            //row.Cells[7].Value = fb.ResultInfo.Follow;
                            //row.Cells[8].Value = fb.ResultInfo.Friend;
                            row.Cells[6].Value = fb.ResultInfo.CreatedTime;
                        }
                    }
                    getlist_.Text = "GET";
                });
        ...
        private void savelist__Click(object sender, EventArgs e)
        {
